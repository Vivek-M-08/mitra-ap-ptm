...
Generate a structured table showing complaint counts grouped by District, School, Category, Issue Theme, Unique User Count, Total Feedback Count, and Complaint Count.
...
4. **Aggregate per group:** For each unique combination of `(District, School, Category, Issue Theme)`, compute:
   ...
   - **Total Feedback Count:** Total number of feedback rows (all ratings, including non-complaints) submitted for that school under that category — this is the denominator for percentage calculations.
   - **Complaint Count:** Total number of complaint rows (including duplicate respondents if they reported the same issue) in that group, expressed as both a whole number and a percentage of the Total Feedback Count for that school/category.
     - Format: `23 (2% of feedbacks)`
5. **Sort:** Order rows by `Complaint Count` descending within each district, so the most frequent issues appear first.
6. **Do not include a Severity or Risk Level column.**

### Required Column Format

| District | School | Category | Issue Theme | Unique User Count | Total Feedbacks | Complaint Count |
|----------|--------|----------|-------------|-------------------|-----------------|-----------------|
| Chittoor | ZPHS School | Drinking Water | Dirty water complaints | 14 | 210 | 18 (9% of feedbacks) |
| Guntur | Govt High School | Cleanliness | Toilets not cleaned | 9 | 145 | 11 (8% of feedbacks) |
...
**Affected Schools:**
List all schools where this challenge was reported, grouped by district. For each school, provide the **total feedback count** and the **complaint count** for that challenge category in the following format:

> `School Name (total feedbacks: N | complaints: X)`

**Example:**
> NANDYAL district: Dr. B. R. Ambedkar Gurukulam LAXMAPURAM (total feedbacks: 19 | complaints: 8), Gurukulam ALLAGADDA (total feedbacks: 15 | complaints: 6), Gurukulam KOILAKUNTLA (total feedbacks: 12 | complaints: 4); Y.S.R. district: Gurukulam CHINNACHOWK (total feedbacks: 22 | complaints: 7), Gurukulam KAMALAPURAM (total feedbacks: 18 | complaints: 5); KURNOOL: Gurukulam KURNOOL (total feedbacks: 14 | complaints: 5); plus additional schools across remaining districts.
...
1. **Aggregate complaints per school:** For each `(District, School)` pair, sum all complaint rows across all categories.
2. **Total Feedbacks:** Count total feedback rows (all ratings) submitted for that school across all categories — this is the denominator.
3. **Complaint Count:** Express as a whole number and as a percentage of Total Feedbacks.
   - Format: `24 (2%)`
4. **Sort:** Order by Complaint Count descending.
5. **Do not compute or include a Risk Level column.**

### Required Columns

| District | School | Total Feedbacks | Complaint Count |
|----------|--------|-----------------|-----------------|
| Nandyal | Dr. B. R. Ambedkar Gurukulam LAXMAPURAM | 210 | 45 (21%) |
...
5. **Total Feedbacks:** For each `(District, School)` pair, count total feedback rows across all categories.
6. **Complaint Count:** Express as a whole number and percentage of Total Feedbacks for that school.
   - Format: `24 (2%)`
7. **Do not assign or include a Priority or Risk Level column.**

### Required Fields

| District | School | Total Feedbacks | Complaint Count |
|----------|--------|-----------------|-----------------|
| Chittoor | ZPHS School | 310 | 28 (9%) |

> Additionally, for internal narrative use (not as additional table columns), track: Category, Rating, Original Comment, Extracted Theme, and Action Required per row. These details should appear in the narrative/supporting text beneath the table, not as separate table columns.
...
4. **Total Feedbacks:** Count all feedback rows (all ratings) for that school under `PRINCIPAL_SATISFACTION`.
5. **Complaint Count:** Count complaint rows and compute as a percentage of Total Feedbacks.
   - Format: `24 (2%)`
6. **Cluster issues:** Extract the most common complaint themes per principal/school group and list them in the `Common Issues` column.
7. **Flag for red-listing:** Include a school/principal in this section only if:
   - Complaint Count ≥ 3, **OR**
   - Any complaint carries a `rating = 1`
8. **Do not compute or include a Risk Level column.**
...
| District | School | Total Feedbacks | Complaint Count |
|----------|--------|-----------------|-----------------|
| Kurnool | Gurukulam KURNOOL | 85 | 12 (14%) |

> Additionally, for each flagged principal/school, list the `Common Issues` as a bullet list beneath the table row (e.g., "Rude behavior", "Absent from school", "Ignores parents").
...
4. **Total Feedbacks:** Count all feedback rows (all ratings) for that school under `CARETAKER_SATISFACTION`.
5. **Complaint Count:** Count complaint rows and compute as a percentage of Total Feedbacks.
   - Format: `24 (2%)`
6. **Cluster issues:** Extract the most common complaint themes per caretaker/school group and list them in the `Common Issues` column.
7. **Flag for red-listing:** Include a school/caretaker in this section only if:
   - Complaint Count ≥ 3, **OR**
   - Any complaint carries a `rating = 1`
8. **Do not compute or include a Risk Level column.**
...
| District | School | Total Feedbacks | Complaint Count |
|----------|--------|-----------------|-----------------|
| Nandyal | Gurukulam ALLAGADDA | 72 | 9 (13%) |

> Additionally, for each flagged caretaker/school, list the `Common Issues` as a bullet list beneath the table row (e.g., "Toilets dirty", "Unresponsive to hygiene issues", "Misbehavior reported").
...
|----------|--------|-----------------|-------------------|
| Chittoor | ZPHS School | 18 (9% of feedbacks) | Foul-smelling water, No supply for days |
...
> **Reminder:** The output must be a `.md` file with **exactly 8 metrics**. Risk Level and Severity columns must not appear anywhere in the output tables.
